# # How the basics of computer chips and memory affects the speed of your code
#
# Programming is used in many fields of science today, where individual scientists often have to write custom code for their own projects. For most scientists, however, computer science is not their field of expertise, instead, they have learned programming by necessity. While these people may be resonably familiar with the *software* side of programming, they rarely have a basic understanding of the *hardware* side.
#
# The aim of this tutorial is to give programmers who are not programmers by profession a *brief* overview of the features of modern hardware that you must understand in order to write fast code. I will use Julia as my language of choice because it allows these relatively low-level considerations to be shown easily in high-level code and in an interactive language.
#
...
# To write fast code *in practise*, you must benchmark it. You must profile your code to find bottlenecks where your machine spends the majority of the time, and you must benchmark different functions and approaches to find the fastest in practise. Julia (and other languages) have tools for exactly this purpose, but I will not cover them here.
...
# * Random access memory (RAM) is the short-term memory of the computer. This memory requires electrical power to maintain, and is lost when the computer is shut down. Computers must have data in RAM in order to work on it. Much of time spent "loading" various applications and operating systems is spent moving data from disk to RAM and unpacking it there. A typical consumer laptop has around 1e11 bits of RAM memory.
# * The disk is a mass storage unit. This data on disk persists after power is show down, so the disk contains the long-term memory of the computer. It is also much cheaper per gigabyte than RAM, with consumer PCs having around 5e12 bits of disk space.
...
#
# When working with data too large to fit into RAM, load in the data chunk by chunk, e.g. one line at a time, and operate on that. That way, you don't need *random access* to your file, but only sequential access.
#
...
#
# Effective use of the cache comes down to *locality*: Your CPU does not copy *just* the needed bytes to cache. Instead, it, your CPU will always copy larger chunk of data (usually consecutive 512 bits).
#
...
println("Size of AlignmentTest: ", sizeof(AlignmentTest))
...
# Let's break it down: The lines beginning with `;` are comments, and explain which section of the code the following instructions come from. For example, in the first 11 code lines:
#
# ```
...
# ```
#
# The comment lines here show the nested series of function calls and where in the source code they are. You can see that `eachindex` here calls `axes1`, which calls `axes`, which calls `size` and `map`, etc. Under the comment line containing the `size` call, we see the first CPU instruction. The instruction name is on the far left, `movq`. The name is composed of two parts, `mov`, the kind of instruction (to move a single integer of data), and a suffix `q`, short for "quad", which means 64-bit integer. There are the following suffixes:  `b` (byte, 8 bit), `w` (word, 16 bit), `l`, (long, 32 bit) and `q` (quad, 64 bit).
#
...
#
# You can also see that the code is segmented into sections beginning with a name starting with "L", for example the last section `L131`. These sections are jumped between using if-statements, or *branches*. For example, the actual loop is marked with `L48`. You can see the following two instructions in the `L48` section:
#
# ```
#     cmpq	%r8, %rcx
# 	jb	L48
...
# Not all CPU instructions are equally fast. If you have an inner loop executing millions of times, it may pay off to inspect the generated assembly code for the loop and check if you can express the computation in terms of fast CPU instructions. Below is a table of selected CPU instructions with *very rough* estimates of how many clock cycles they take to execute.
#
...
# ```
#
# For example, if you have an integer you know to be 0 or above, and you want to divide it by 8 (discarding any remainder), you can instead do a bitshift:

divide_slow(x) = div(x, 8)
divide_fast(x) = x >>> 3
#----------------------------------------------------------------------------

# However, modern compilers are pretty clever, and will often figure out the optimal instructions to use in your functions to obtain the same result, by for example replacing an integer divide `idivq` instruction with bitshifts (`shrq` or `shlxq`) where applicable to be faster. You need to check the assembly code yourself to see:
...
# Here, two 4\*64 bit vectors are added together in one single instruction. You can see the CPU makes use of a single `vpaddd` (vector packed add double) instruction to add 4 integers, as well as the corresponding move instruction `vmovdqu`. Note that vector CPU instructions begin with `v`.
#
# ### Automatic vectorization
# SIMD vectorization of 64-bit integers may increase throughput by almost 4x, so it is of huge importance in high-performance programming. Compilers will automatically vectorize operations if they can. What can prevent this automatic vectorization?
#
...
#
# A good rule of thumb is that you should not have any branches (i.e. if-statements) in the loop at all if you want it to SIMD-vectorize. In fact, even boundschecking, i.e. checking that you are not indexing outside the bounds of a vector, causes a branch. To achieve SIMD vectorization then, all boundschecks must be disabled. We can use this do demonstrate the impact of SIMD:
...
#
# The latest CPUs contain specialized instructions for AES encryption and SHA256 hashing. If you wish to call these instructions, you can call Julia's backend compiler, LLVM, directly. In the example below, I create a function which calls the `vaesenc` (one round of AEC encryption) instruction directly:
...
# The function `call_plus` calls `+`, and is compiled to a single `leaq` instruction (as well as some filler `retq` and `nopw`). But `+` is a normal Julia function, so `call_plus` is an example of one regular Julia function calling another. So why is there no `callq` instruction?
#
...
#
# Instead, the compiler uses heuristics (rules of thumb) to determine when a function is small enough for inlining to increase performance. These heuristics are not bulletproof, so Julia provides the macros `@noinline`, which prevents inlining of small functions, and `@inline`, which does not *force* the compiler to inline, but *strongly suggests* to the compiler that it ought to inline the function.
#
...
#
# Branches caused by loops are among the easiest to guess. If you have a loop with 1000 elements, the code will loop back 999 times and fail to loop back just once. Hence the branch predictor can simply always predict "loop back", and get a 99.9% accuracy.
#
# We can demonstrate this with a simple function:
...
# Which contain no other branches than the one caused by the loop itself (which is easily predictable), and results in speeds similar to the perfectly predicted example above, but even for random data.
#
